...
class Metric():
    ...
    def update(self, values):
        ...
        for i in range(len(values)):
            ...
            if self.metrics[i][0] == None:
                self.metrics[i][0] = values[i]
                self.metrics[i][1] = values[i]
                self.metrics[i][2] = values[i]
            else:
                self.metrics[i][0] = (self.metrics[i][0] * (self.num_iters - 1) + values[i]) / self.num_iters
                self.metrics[i][1] = 0.95 * self.metrics[i][1] + 0.05 * values[i]
                self.metrics[i][2] = values[i]
