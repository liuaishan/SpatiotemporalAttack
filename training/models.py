...
class VqaLstmCnnAttentionModel(nn.Module):
    ...
    def render(self, pos, v, f, vt):      
        self.renderer.t_perspective = 30
        robot_height = 1.0
        self.renderer.eye = (pos[0], robot_height, pos[2]) 
        yaw = pos[3]        
        pitch = 0
        front = [(math.cos(yaw*math.pi/180)) * (math.cos(pitch*math.pi/180)), math.sin(pitch*math.pi/180),
        (math.sin(yaw*math.pi/180)) * (math.cos(pitch*math.pi/180))]
        sf = math.sqrt(math.pow(front[0],2) + math.pow(front[1],2) + math.pow(front[2],2))
        self.renderer.t_at = [front[0]/sf + pos[0], front[1]/sf + robot_height, front[2]/sf + pos[2]]
        images, _, _ = self.renderer(v, f, vt)  # [batch_size, RGB, image_size, image_size]
        image = images[0]
        return image


    
    def get_frames(self, v, f, vt, pos_queue):
        if isinstance(pos_queue, list) == False:
            pos_queue = [pos_queue]
        res = []     
        for i in range(len(pos_queue)):
            pos = pos_queue[i]
            img = self.render(pos, v, f, vt)
            
            res.append(img)
        return torch.stack(res)
    
    def get_frames_batch(self, v, f, vt, pos_queue, save, q_idx):
        batch_frame = []
        for idx in range(v.shape[0]):
            image = self.get_frames(
                    v[idx], f[idx], vt[idx],
                    pos_queue[idx])
            batch_frame.append(image)
        return torch.stack(batch_frame)

    def forward(self, v_var, f_var, vt_var, pos, questions, save, q_idx):
        pos_queue = pos
        images = self.get_frames_batch(
                    v_var, f_var, vt_var,
                    pos_queue,
                    save,
                    q_idx)
